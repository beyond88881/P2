...
$(function () {
    // var w_show = document.documentElement.scrollWidth || document.body.scrollWidth;//网页正文全文宽高
    // var h_show = document.documentElement.scrollHeight || document.body.scrollHeight;//网页正文全文宽高
    // var w_show2 = document.documentElement.offsetWidth || document.body.offsetWidth ;//网页可见区域宽高，包括滚动条等边线（会随窗口的显示大小改变）
    // var h_show2 = document.documentElement.offsetHeight || document.body.offsetHeight ;//网页可见区域宽高，包括滚动条等边线（会随窗口的显示大小改变）
    // var pageWidth=document.body.clientWidth;
	// var pageHeight=document.body.clientHeight;
	var pageWidth=window.innerWidth;
	var pageHeight=window.innerHeight;

    // var w_show3=(h_show-h_show2)
    // $('#index_layout').css("height",window.innerHeight-100 + "px");
    // $('#index_layout').css("height",pageHeight-50 - 30 + "px")//.css("width",pageWidth-100 +"px").css("left","100px");
...
function logout() {
    $.iMessager.confirm('提示', '确定要退出吗?', function (r) {
        if (r) {
            ...
            window.location.href = '/logout'//'./login.html' + location.search;
        }
    });
}
...
function generateMenu(menuId, systemName) {
    if (menuId < 8000) {
        ...
        // var url = "/static/HTML/json/menu/menu_" + menuId + ".json";
        var url = $('#RightAccordion').data("urlbs") + "?menu=" + menuId;//"{% url 'p_general.jsonInfo' menuId  %}";
        ...
        $.get(
            ...
            function (data) {
                ...
                $.each(data, function (i, e) {// 循环创建手风琴的项
                    ...
                    //$.parser.parse();
                    // var secondUrl = "/static/HTML/json/menu/menu_" + e.id + ".json";
                    var secondUrl = $('#RightAccordion').data("urlbs") + "?menu=" + e.id;//"{% url 'p_general.jsonInfo' e.id %}";
                    ...
                    $.get(secondUrl, function (data) {// 循环创建树的项
                        $("#tree" + e.id).tree({
                            ...
                            onBeforeExpand: function (node, param) {
                                // var thirdUrl = "/static/HTML/json/menu/menu_" + node.id + ".json";
                                var thirdUrl = $('#RightAccordion').data("urlbs") + "?menu=" + node.id;//"{% url 'p_general.jsonInfo'" + node.id + ".json %}";
...
function modifyPwd() {
    var opts = {
        ...
        height: 300,
        iconCls: 'fa fa-key',
        href: '/modifyPassword/', //'./html/user/modifyPassword.html',
        buttons: [{
            text: '确定',
            iconCls: 'fa fa-save',
